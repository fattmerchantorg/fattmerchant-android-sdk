...
import com.fattmerchant.cpresent.omni.entity.models.Merchant
...
class ChipDnaDriver : CoroutineScope, MobileReaderDriver {
    ...
    class ConnectReaderException(): OmniException("Could not connect mobile reader") {

        constructor(detail: String? = null): this() {
            if (detail != null) {
                this.detail = mapDetailMessage(detail)
            }
        }
    ...
    override suspend fun initialize(args: Map<String, Any>): Boolean {
        val appContext = args["appContext"] as? Context
            ?: throw Error("Could not initialize reader. appContext not found")

        val appId = args["appId"] as? String
            ?: throw Error("Couldnt not initialize reader. appId not found")

        val merchant = args["merchant"] as? Merchant
            ?: throw Error("Could not initialize reader. Merchant not found")

        val apiKey = merchant.emvPassword()
            ?: throw Error("Could not initialize reader. emvTerminalSecret not found")
        ...
        if (result[ParameterKeys.Result] != ParameterValues.TRUE) {
            throw Error("Could not initialize reader. Invalid credentials for reader")
        }
...
//        val pinpadName = if (status.containsKey(ParameterKeys.PinPadName)) {
//            status.getValue(ParameterKeys.PinPadName)
//        } else {
//            return false
//        }
//
//        if (reader.getName() != pinpadName) {
//            return false
//        }
        ...
        val result = suspendCancellableCoroutine<Boolean> { cont ->
            log("About to call connect and configure")
        ...
        if (result.containsKey(ParameterKeys.Errors)) {
            ...
            when {
                result[ParameterKeys.Errors]?.contains("PinPadUserCancelled") == true -> {
                    throw Error("User cancelled transaction")
                }

                result[ParameterKeys.Errors]?.contains("InvalidCardResponse") == true -> {
                    throw Error("Invalid card response")
                }

                else -> {
                    throw Error(result[ParameterKeys.ErrorDescription] ?: "Unknown error performing mobile reader transaction")
                }
            }
        }
        ...
        if (result[ParameterKeys.TransactionResult] == ParameterValues.Declined) {
            when {
                result[ParameterKeys.Errors]?.contains("GatewayRejectedTransaction") == true -> {
                    throw Error("Gateway rejected transaction")
                }
            }
        }
    ...
    companion object {
         ...
         */
        private fun extractUserReference(transaction: Transaction): String? = (transaction.meta as? Map<*, *>)?.get("nmiUserRef") as? String
         ...
         */
        private fun generateUserReference(): String = String.format("CDM-%s", SimpleDateFormat("yy-MM-dd-HH.mm.ss", Locale.US).format(Date()))
        ...
        private fun mapTransactionRequestToParams(request: TransactionRequest) = Parameters().apply {
            ...
            add(ParameterKeys.Currency, "USD") //TODO: Support other currencies
            add(ParameterKeys.UserReference, generateUserReference())
            add(ParameterKeys.PaymentMethod, ParameterValues.Card)
            add(ParameterKeys.AutoConfirm, ParameterValues.TRUE)

            val type = when (request.type) {
                TransactionType.SALE -> ParameterValues.Sale
                TransactionType.REFUND, TransactionType.VOID-> ParameterValues.Refund
            }
            add(ParameterKeys.TransactionType, type)
        }
    }
}